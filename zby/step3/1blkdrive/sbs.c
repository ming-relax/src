...
{
	int i;
	unsigned long l_flag;
	char buf[1024];
	set_current_state(TASK_INTERRUPTIBLE);
	spin_lock_irqsave(&io_counter.lock, l_flag);
	for(i=0; i < 100; i ++){
		printk(KERN_NOTICE "id: %d, io count: %d.\n", i, atomic_read(&io_counter.cnt));
		memset(buf, i, 1024);
	}
	spin_unlock_irqrestore(&io_counter.lock, l_flag);
}
...
int my_bio_callback (struct bio *bio, unsigned int bytes_done, int error)
{
	 my_ctx_t * ctx = bio->bi_private;
	 dec_io_counter();
...
void change_callback_for_my_bio(struct bio* bio)
{
	my_ctx_t * ctx = kmalloc(sizeof(my_ctx_t), GFP_KERNEL);	

	ctx->private = bio->bi_private;
	ctx->callback = bio->bi_end_io;
	bio->bi_private = ctx;
	bio->bi_end_io = my_bio_callback;
...
static int sbs_make_request(request_queue_t *q, struct bio *bio)
{
	...
	if(bio_data_dir(bio) == WRITE){
		printk(KERN_NOTICE  "change write bio's callback \n");
		for( i =0; i < 1000; i ++){
			change_callback_for_my_bio(bio);
		}
	}
	return 1;
}
