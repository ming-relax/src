...
pub struct Scheduler<T> {
    name: Arc<String>,//DHQ: Arc的clone，只产生一个新的pointer，不copy数据区
...
impl<T: Display> Scheduler<T> {
    fn new<S: Into<String>>(
        name: S,
        counter: AtomicUsize,
        sender: Sender<Option<T>>,//DHQ: Sender用到了T
    ) -> Scheduler<T> {
        Scheduler {
...
impl<T: Display> Clone for Scheduler<T> {
    fn clone(&self) -> Scheduler<T> {//DHQ:Clone是个trait，不要求copy所有的field。
        Scheduler {
...
pub struct Worker<T: Display> {
    ...
    receiver: Mutex<Option<Receiver<Option<T>>>>,//DHQ: TODO: 为什么需要一个Mutex?
    ...
    while keep_going {
        ...
        while buffer.len() < batch_size {
            match rx.try_recv() {
                ...
                Ok(Some(t)) => buffer.push(t),//DHQ:不是直接处理，而是放到buffer(vector)中,下面run_batch
                _ => break,
            }
        }
        ...
        WORKER_HANDLED_TASK_VEC
            ...
            .unwrap();
        runner.run_batch(&mut buffer);//runner有BatchRunnable trait，调用其run_batch, 本文件中
        buffer.clear();
    }
    runner.shutdown();
}
...
impl<T: Display + Send + 'static> Worker<T> {
    ...
    pub fn new<S: Into<String>>(name: S) -> Worker<T> {
        ...
        Worker {
            scheduler: Scheduler::new(name, AtomicUsize::new(0), tx),//DHQ: 就是本文件前面定义的Scheduler的new()
            receiver: Mutex::new(Some(rx)),//DHQ: Mutex::new()，而不是直接使用了
            handle: None,
        }
    }
    ...
    where
        R: BatchRunnable<T> + Send + 'static,
    {
        let mut receiver = self.receiver.lock().unwrap();//DHQ: lock，unwrap出receiver,这个函数调用会并发么？
        ...
        let h = Builder::new()
            ...
            .spawn(move || poll(runner, rx, counter, batch_size))?; //DHQ: poll, worker/mod.rs中，循环recv，调用runner处理. rx的ownership应该就move给thread了
        self.handle = Some(h);
        Ok(())
    }
...
mod test {
    ...
    impl BatchRunnable<u64> for BatchRunner {//DHQ: 这段是测试用的
