...
//DHQ: node.rs, start_store函数，初始化和运行
    ...
    C: PdClient,
{
    ...
    config.timer_tick_ms(cfg.raft_base_tick_interval.as_millis() / MIO_TICK_RATIO);//DHQ: 默认值1s，除完还是 100ms
...
impl<T: Transport, C: PdClient> Store<T, C> {
    pub fn run(&mut self, event_loop: &mut EventLoop<Self>) -> Result<()> {
        ...
        box_try!(self.region_worker.start(runner));//DHQ: worker跟runner关联？ TODO: worker是个单独线程
    ...
    fn register_raft_base_tick(&self, event_loop: &mut EventLoop<Self>) {
        ...
        info!("DHQ: register raft base tick :{},ms{}", self.tag, self.cfg.raft_base_tick_interval.as_millis());
    ...
    fn on_raft_base_tick(&mut self, event_loop: &mut EventLoop<Self>) {
        let timer = self.raft_metrics.process_tick.start_coarse_timer();//DHQ: new timer
        for peer in &mut self.region_peers.values_mut() {//DHQ: An iterator visiting all values mutably in arbitrary order.
        ...
        timer.observe_duration();//DHQ: 这里flush的是 process_tick 这个metric 
    ...
    fn poll_apply(&mut self) {//DHQ: tick()会调用
        loop {
    ...
    fn on_raft_message(&mut self, mut msg: RaftMessage) -> Result<()> {
        ...
        if !self.maybe_create_peer(region_id, &msg)? {//DHQ: 这个函数里面可能创建新peer
            return Ok(());
        }
        ...
        peer.insert_peer_cache(msg.take_from_peer());//DHQ: insert to cache
    ...
    fn on_raft_ready(&mut self) {//DHQ: 处理整个store的Ready
        ...
        if !region_proposals.is_empty() { 
            ...
            // messages ASAP.
            self.trans.flush(); //DHQ: 对应leader的消息，先发送出去
        }
        ...
        if !kv_wb.is_empty() {//DHQ: 这里才分别写盘，kv_wb, raft_wb都是需要写disk的
        ...
        if !raft_wb.is_empty() {
            ...
            let mut write_opts = WriteOptions::new();
            write_opts.set_sync(self.cfg.sync_log || sync_log);//sync方式写，写的raft log？
            self.raft_engine
        ...
        slow_log!(//DHQ: 这个里面，是不是可以看出每次处理了多少个group?
            t,
    ...
    fn propose_raft_command(&mut self, msg: RaftCmdRequest, cb: Callback) {
        match self.pre_propose_raft_command(&msg) {//DHQ:没看到里面有网络操作
            Ok(Some(resp)) => {
                cb.call_box((resp,));
                return;
            }
        ...
        if peer.propose(cb, msg, resp, &mut self.raft_metrics.propose) {//DHQ: peer.rs有propose实现
            peer.mark_to_be_checked(&mut self.pending_raft_groups);
        }
    ...
    fn on_pd_heartbeat_tick(&mut self, event_loop: &mut EventLoop<Self>) {
        for peer in self.region_peers.values_mut() {//DHQ: 一个store有多个region(peer)，各个peer逐个检查
            peer.check_peers();
        }
        let mut leader_count = 0;
        for peer in self.region_peers.values() {
            if peer.is_leader() {//DHQ: 只对leader做？ pd的heartbeat
                leader_count += 1;
...
fn register_timer<T: Transport, C: PdClient>(
    ...
    if let Err(e) = event_loop.timeout_ms(tick, delay) {
        return Err(box_err!(
            "failed to register timeout [{:?}, delay: {:?}ms]: {:?}",j
            tick,
            delay,
            e
        ));
...
//DHQ: struct T必须实现trait Transport, C需要实现trait PdClient
//
impl<T: Transport, C: PdClient> mio::Handler for Store<T, C> {//DHQ: 这几个是mio的handler实现，由mio层调用
    type Timeout = Tick;
    type Message = Msg;
    ...
    fn notify(&mut self, event_loop: &mut EventLoop<Self>, msg: Msg) {//DHQ: 参见 mio的run_once，这个函数会调用Handler的notify
        match msg {
            ...
            Msg::BatchRaftSnapCmds {
                ...
                batch,
                on_finished,
            } => {//DHQ: 这个observer没有注释，但是也没有sys_gettime了，说明根本走不到。
                self.raft_metrics
                    .propose
                    .request_wait_time
                    .observe(duration_to_sec(send_time.elapsed()) as f64);
    ...
    fn timeout(&mut self, event_loop: &mut EventLoop<Self>, timeout: Tick) {//DHQ: mio的timer_process调用，各种timer在里面
        let t = SlowTimer::new();
        match timeout {
    ...
    fn tick(&mut self, event_loop: &mut EventLoop<Self>) {//DHQ: 这个函数应该是mio要求实现的，event_loop.rs中的 handler.tick(self)。
