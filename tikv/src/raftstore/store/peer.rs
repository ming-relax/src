...
impl ReadIndexQueue {//DHQ:从Queue分id，给ReadIndexRequest
...
impl<'a, T> ReadyContext<'a, T> {//DHQ: raft_ready的保存结构
...
impl Peer {
    ...
    fn new<T, C>(store: &mut Store<T, C>, region: &metapb::Region, peer_id: u64) -> Result<Peer> {
        ...
        let raft_group = RawNode::new(&raft_cfg, ps, &[])?;//DHQ: raft_group是个raw_nodw，使用storage为ps，即PeerStorage
	...
	//DHQ: 这个region是发送过来的region，不是本地结构体里面的。https://segmentfault.com/a/1190000008007027
    ...
    pub fn step(&mut self, m: eraftpb::Message) -> Result<()> {
        ...
        self.raft_group.step(m)?;//DHQ: 注释为，可能因为term旧而step down
        Ok(())
    }

    pub fn check_peers(&mut self) {//DHQ: 参见调用处, store对各个region/peer依次调用
        if !self.is_leader() {//DHQ: 不是leader，不主动做heartbeat?
    ...
    pub fn collect_down_peers(&self, max_duration: Duration) -> Vec<PeerStats> {
        ...
        for p in self.region().get_peers() {
            ...
            if let Some(instant) = self.peer_heartbeats.get(&p.get_id()) {
                if instant.elapsed() >= max_duration {//DHQ: 检查超时时间确认
    ...
    pub fn check_stale_state(&mut self, d: Duration, cur: Instant) -> StaleState {
    ...
    pub fn handle_raft_ready_apply(&mut self, mut ready: Ready, apply_tasks: &mut Vec<Apply>) {
        ...
        if self.is_applying_snapshot() {
            ...
            for entry in committed_entries.iter().rev() {
                ...
                if to_be_updated {
                    to_be_updated = !self.maybe_update_lease(entry);//DHQ: TODO，修改了有啥用?
                }
            }
    ...
    fn apply_reads(&mut self, ready: &Ready) {
        ...
        if self.ready_to_handle_read() {
            for state in &ready.read_states {
                ...
                for (req, cb) in read.cmds.drain(..) {
                    ...
                    // procedure precisely.
                    cb(self.handle_read(req));//DHQ: 这个里面handle_read确实会去读，并获取结果
                }
                propose_time = Some(read.renew_lease_time);
            }
        } else {//DHQ: 当前不能处理读,可能是leader change导致的
            for state in &ready.read_states {
        ...
        // actually stale.
        if ready.ss.is_some() {//DHQ: 跟soft state有啥关系？
            let term = self.term();
    ...
    pub fn post_apply(
        ...
        if self.pending_reads.ready_cnt > 0 && self.ready_to_handle_read() {
            for _ in 0..self.pending_reads.ready_cnt {
                let mut read = self.pending_reads.reads.pop_front().unwrap();
                for (req, cb) in read.cmds.drain(..) {
                    cb(self.handle_read(req));//DHQ: 这个里面确实有snapshot read
                }
            }
    ...
    fn update_lease_with(&mut self, propose_time: Timespec) {
        ...
        if self.leader_lease_expired_time.is_some() {//DHQ: leader的lease已经超时了(我不是leader)
    ...
    pub fn propose(
        ...
        let res = match self.get_handle_policy(&req) {
            Ok(RequestPolicy::ReadLocal) => {
                self.read_local(req, cb, metrics);//DHQ: 无需网络，直接本地读
                return false;
            }
    ...
    fn get_handle_policy(&mut self, req: &RaftCmdRequest) -> Result<RequestPolicy> {
        ...
        if is_write {
            return Ok(RequestPolicy::ProposeNormal);//DHQ: 正常的raft写
        }
        ...
        if let Some(Either::Left(safe_expired_time)) = self.leader_lease_expired_time {
            if monotonic_raw_now() <= safe_expired_time {
                return Ok(RequestPolicy::ReadLocal);//DHQ: lease还在，本地读即可
            }
		//DHQ: 未超时则上面return了
            debug!(
