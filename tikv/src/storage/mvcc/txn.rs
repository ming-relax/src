...
impl MvccTxn {
    ...
    fn lock_key(
        ...
        self.writes.push(Modify::Put(CF_LOCK, key, lock));//DHQ: Percolator的锁类似，在commit时还要检查。TODO: 这里到底能否写/加锁成功？
    }
    ...
    pub fn commit(&mut self, key: &Key, commit_ts: u64) -> Result<()> {
        let (lock_type, short_value) = match self.reader.load_lock(key)? {//DHQ: 检查是否自己lock时的版本. ?表示如果失败，直接返回。
