...
pub type Callback<T> = Box<FnBox((CbContext, Result<T>)) + Send>;//DHQ: Box是个pointer的意思，这个就是说函数指针，只不过指向的结构体支持Send, 可以转移ownership。 参考 std::boxed::Box
