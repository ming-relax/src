    ...
    C: PdClient,
{
    ...
    pub fn start<T>(//DHQ: tikv_server.rs中，run_raft_server() 调用
        &mut self,
    ...
    #[allow(too_many_arguments)]
    fn start_store<T>(//DHQ: 这个应该是让 raft_store干活的地方
        &mut self,
        ...
        T: Transport + 'static,
    {
        ...
        let h = builder.spawn(move || {
            ...
            let mut store = match Store::new(//DHQ: raftstore/store/store.rs里面
                ch,
                store,
                cfg,
                engines,
